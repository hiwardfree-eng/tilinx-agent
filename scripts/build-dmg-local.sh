...
# already-built TilinX.app and re-packages it into a styled DMG in
...
#      `target/universal-apple-darwin/release/bundle/macos/TilinX.app`,
...
#   ./scripts/build-dmg-local.sh path/to/TilinX.app
#
# Output:
#   target/local-dmg/TilinX-local.dmg (volume name: "TilinX Installer")
#
# Iteration loop:
...
if [ -z "$APP" ]; then
    for candidate in \
        "target/universal-apple-darwin/release/bundle/macos/TilinX.app" \
        "target/aarch64-apple-darwin/release/bundle/macos/TilinX.app" \
        "target/x86_64-apple-darwin/release/bundle/macos/TilinX.app" \
        "target/release/bundle/macos/TilinX.app"; do
        if [ -d "$candidate" ]; then
            APP="$candidate"
            break
        fi
...
if [ -z "$APP" ] || [ ! -d "$APP" ]; then
    echo "::error::No TilinX.app found." >&2
    echo "Either:" >&2
    echo "  - run 'cd app && pnpm tauri build' once first, OR" >&2
    echo "  - pass the .app path: $0 path/to/TilinX.app" >&2
    exit 1
fi
...
APPLICATIONS_Y=258
ICON_SIZE=110
VOLUME_NAME="TilinX Installer"

OUT_DIR="target/local-dmg"
OUT_DMG="$OUT_DIR/TilinX-local.dmg"
mkdir -p "$OUT_DIR"
rm -f "$OUT_DMG"
...
tell application "Finder"
    tell disk "$VOLUME_NAME"
        ...
        set position of item "TilinX.app" of container window to {$APP_X, $APP_Y}
...
echo "  1. DON'T drag the app out — instead double-click TilinX.app inside the mounted disk."
echo "  2. The 'Move TilinX to your Applications folder' dialog should appear."
