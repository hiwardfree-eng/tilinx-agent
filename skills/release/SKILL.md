---
name: release
description: Ship new TilinX version. Bump semver across all packages, tag, push, let CI build + sign + notarize. STOP at draft — user publishes manually. Defaults patch bump. Minor needs explicit user permission.
---
...
1. Builds `tilinx-engine` for BOTH `aarch64-apple-darwin` AND `x86_64-apple-darwin`
...
7. **Wait CI:** ~10-15 min. Check `github.com/gettilinx/tilinx/actions`
...
9. **STOP. Hand off draft to user.** The CI-created GH Release is a draft. Tell the user it's ready for review and link it. Do NOT publish it yourself — `gh release edit --draft=false` is the user's call. Publishing flips on auto-update for every installed TilinX, so it's never auto-pilot.
