...
Source of truth for every `?utm_*=` parameter we ship in a TilinX link.
...
```
https://gettilinx.ai/?utm_source=email&utm_medium=email&utm_campaign=launch_v0_4_13&utm_content=cta_header
```

### QR at a YC Demo Day event
```
https://gettilinx.ai/?utm_source=qr_code&utm_medium=event&utm_campaign=yc_demo_day_2026&utm_content=qr_table_tent
```

### Follow-up email to YC Demo Day attendees
```
https://gettilinx.ai/?utm_source=email&utm_medium=email_followup&utm_campaign=yc_demo_day_2026&utm_content=irl_followup_24h
```
...
### Twitter post
```
https://gettilinx.ai/?utm_source=twitter&utm_medium=social&utm_campaign=launch_v0_4_13
```

### Producthunt launch day
```
https://gettilinx.ai/?utm_source=producthunt&utm_medium=referral&utm_campaign=producthunt_launch_2026
```
...
1. **Person scans the QR** at the event → lands on `gettilinx.ai/yc-demo-day-2026` (the short slug)
2. **Cloudflare Pages reads `_redirects`** → 302 to `gettilinx.ai/?utm_*` with the full UTM params
...
5. **Person installs TilinX, opens it for the first time**
6. **TilinX desktop app, on first launch only** (`isNew=true` from the install_id cache in `app/src/lib/install-id.ts`): opens `https://gettilinx.ai/welcome?install_id=<id>` in the user's default browser via `tauriSystem.openUrl`
