# TilinX Engine API (for the webapp)
...
client live in **`@tilinx/runtime-client`** — prefer that over hand-rolling fetch.
...
- If the engine is started with `TILINX_RUNTIME_TOKEN`, send `Authorization: Bearer <token>`
...
auth is a bearer token, not a cookie). Lock down with `TILINX_CORS_ORIGIN=https://app.example.com`.
...
All request/response shapes are exported types from `@tilinx/runtime-client`
...
`@tilinx/runtime-client` is a pnpm workspace package (zero runtime deps),
consumed as TypeScript source, no build step. From the webapp:

```bash
pnpm add @tilinx/runtime-client@workspace:*
```
