...
const outputs = [
  "css/tokens.css",
  "ts/tokens.ts",
  "swift/TilinXTokens.swift",
  "kotlin/TilinXTokens.kt",
];

const fresh = mkdtempSync(join(tmpdir(), "tilinx-tokens-sync-"));
...
describe("committed dist is in sync with source", () => {
  for (const rel of outputs) {
    it(`${rel} matches a fresh build`, () => {
      ...
      expect(
        committed,
        `dist/${rel} is stale — run: pnpm --filter @tilinx/design-tokens build`,
      ).toBe(rebuilt);
    });
  }
});
