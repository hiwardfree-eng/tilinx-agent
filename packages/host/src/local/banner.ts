/**
 * The `TILINX_HOST_LISTENING` startup banner.
 *
 ...
 *    (`TILINX_HOST_TOKEN`) by an orchestrator that already knows it. Nothing
 ...
 *
 * Both forms keep the `TILINX_HOST_LISTENING port=<p>` prefix intact, which is
...
export function formatHostListeningBanner(args: {
  ...
  const base = `TILINX_HOST_LISTENING port=${args.port}`;
