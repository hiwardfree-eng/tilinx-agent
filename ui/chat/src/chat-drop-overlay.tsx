...
export function ChatDropOverlay({
  ...
  return (
    <div
      className="pointer-events-none absolute inset-0 z-20 flex items-center justify-center bg-input/80 backdrop-blur-sm"
      aria-hidden="true"
    >
      <div className="flex w-full max-w-sm -translate-y-12 flex-col items-center gap-3 px-6 text-center">
        <FilesIcon className="size-8 text-ink-muted" strokeWidth={1.5} />
        <div className="text-2xl font-semibold tracking-tight text-ink">
          {title}
        </div>
        <p className="text-sm/relaxed text-ink-muted">{description}</p>
      </div>
    </div>
  );
}
