/**
 * TilinXAvatar — the colored TilinX helmet glyph, optionally wrapped in
 ...
 * across every TilinX surface (desktop, mobile, any third-party
 * frontend built on `tilinx-engine`). Old local copies in `app/` and
 ...
 * so the host must load `@tilinx/design-tokens`; without the core globals the
...
const TILINX_GRAY = "var(--ht-ink-muted)";

interface HelmetProps {
  /** CSS fill color (hex, currentColor, or a var() reference — agent palette
   *  colors arrive as var(--ht-agent-*) so they track the theme live).
   *  Defaults to TilinX gray. */
  color?: string;
...
/** Bare TilinX helmet SVG. No container, no halo — just the glyph. */
export function TilinXHelmet({
  color = TILINX_GRAY,
  size = 24,
  className,
}: HelmetProps) {
  return (
    ...
    >
      ...
      <rect
        width="15.334"
        height="16.211"
        transform="translate(258.6 409.939) rotate(-89.717)"
      />
...
/** Agent avatar badge: colored circle + TilinX helmet. Flip `running`
 *  to `true` and the badge grows a spinning comet border without any
 *  other code change required. */
export function TilinXAvatar({
  color,
  diameter = 40,
  running = false,
  className,
}: TilinXAvatarProps) {
  const bg = color ?? TILINX_GRAY;
  ...
  const inner = (
    ...
    >
      <TilinXHelmet color={bg} size={Math.round(innerDiameter * 0.65)} />
    </div>
  );
