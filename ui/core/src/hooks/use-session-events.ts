...
/**
 * Base hook for subscribing to tilinx-tauri backend events.
 *
 * Uses a ref-based handler pattern to avoid the race condition in
 * useTilinXEvent (where handler recreation tears down and re-registers
 * the listener, causing missed events).
 *
 * Apps pass their own `listen` function from `@tauri-apps/api/event`
 * so @tilinx-ai/core doesn't need a build-time dependency on Tauri.
 *
...
import type { TilinXEvent } from "../types";
...
/**
 * Subscribe to "tilinx-event" from the Rust backend.
 *
 * Core events handled:
...
export function useSessionEvents(handlers: SessionEventsHandlers): void {
  ...
  useEffect(() => {
    const unlisten = ref.current.listen<TilinXEvent>(
      "tilinx-event",
      (event) => {
        const h = ref.current;
