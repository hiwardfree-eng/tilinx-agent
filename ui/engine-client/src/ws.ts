 ...
 * ```
 *
 * The engine topic map (see `engine/tilinx-engine-protocol/src/lib.rs::event_topic`):
 * - `*` — firehose; matches every topic below (see
 *    `engine/tilinx-engine-server/src/ws.rs::is_subscribed`)
...
import type { TilinXClient } from "./client.ts";
...
export class EngineWebSocket {
  ...
  constructor(private client: TilinXClient) {}
  ...
  /**
   * Typed event handler — called once per `TilinXEvent` (kind:"event"
