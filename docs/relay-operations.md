...
The TilinX relay (`tilinx-relay/`) is a single Cloudflare Worker + Durable Object that brokers mobile ↔ desktop traffic and serves the mobile PWA bundle from the same origin. This doc covers deployment, secret management, and health checks.
...
cd ../tilinx-relay && pnpm dlx wrangler deploy --env staging
# → tunnel-staging.gettilinx.ai
```

Production:
```
cd mobile && pnpm build
cd ../tilinx-relay && pnpm dlx wrangler deploy
# → tunnel.gettilinx.ai
```
...
Both `tunnel.gettilinx.ai` and `tunnel-staging.gettilinx.ai` are registered as Cloudflare Custom Domains (`custom_domain = true` in `wrangler.toml`). Wrangler auto-provisions the proxied DNS record and certificate. First deploy of a new environment may take ~60s for DNS propagation.
...
8. Restart the Mac and reopen TilinX. Phone works with the existing localStorage token once the tunnel reconnects.
...
- Metrics: Cloudflare dashboard → Workers → `tilinx-relay` → Analytics.
...
```
cd tilinx-relay && pnpm dlx wrangler dev    # → http://localhost:8787
cd ../app && TILINX_TUNNEL_URL=http://localhost:8787 pnpm tauri dev
```

The engine auto-switches WS scheme from `wss://` to `ws://` when `TILINX_TUNNEL_URL` uses `http://`.
