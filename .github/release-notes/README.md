...
single notes string as a trailing `<!--tilinx-i18n:{...}-->` comment.
Every renderer that doesn't understand the comment (GitHub, Slack, older
TilinX builds) drops it and shows the English body; the in-app updater
...
2. Bump version in `app/src-tauri/Cargo.toml`, `app/tilinx-tauri/Cargo.toml`,
