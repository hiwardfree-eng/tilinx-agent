...
- **OpenAI device-code sign-in.** Signing in to OpenAI now works in setups where TilinX cannot open a browser for you, such as a remote engine, the web app, or a headless server. TilinX shows a short code and a URL; open the URL on any device, paste the code, and you are signed in.
...
- **Recover from a corrupt routine history.** If a saved routine run history gets damaged, TilinX now repairs it on its own so the routine can run again instead of getting stuck.
