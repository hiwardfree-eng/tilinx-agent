...
- **Clearer problem reports.** When TilinX hits an error, the follow-up message now says "report sent" with a short reference code. Tap Copy to grab it and send it our way so we can pin down exactly what happened.
...
- **Problem reports carry the real details (behind the scenes).** We fixed our build pipeline so crash reports point at the actual place a problem happened instead of unreadable scrambled code, and so problems in TilinX's background engine get reported too. Nothing for you to do.
