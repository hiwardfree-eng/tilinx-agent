...
- **Conversations that keep going.** When a chat gets close to full, TilinX now compacts it automatically so the conversation can continue without losing the thread or making you start over.
- **Your language stays put.** The interface language you pick is now remembered for each workspace, so it stays the way you set it next time you open TilinX.
