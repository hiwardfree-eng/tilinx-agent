...
/// The JavaScriptCore host for `@tilinx/sdk`.
///
/// Loads the bundled `tilinx-sdk.bridge.js`, installs the host polyfills the
/// bare engine needs (BRIDGE.md §10), calls `TilinXSdkBridge.create({ send })`,
...
final class JSRuntime {
    ...
    static let bundleResourceName = "tilinx-sdk.bridge"
    private static let bundleResourceExt = "js"

    init(queueLabel: String = "ai.tilinx.js") {
    ...
    ///
    /// - Parameters:
    ///   - bundle: bundle holding `tilinx-sdk.bridge.js` (default `.main`).
    ...
    ///   the bundle fails to evaluate / expose `TilinXSdkBridge`.
    func load(
    ...
    private func setUp(
        ...
        context.setObject(send, forKeyedSubscript: "__tilinxSend" as NSString)

        context.evaluateScript(source)
        if let setupException {
            throw JSRuntimeError.evaluationFailed(message: setupException)
        }
        guard let bridgeGlobal = context.objectForKeyedSubscript("TilinXSdkBridge"),
              !bridgeGlobal.isUndefined, !bridgeGlobal.isNull else {
            throw JSRuntimeError.bridgeGlobalMissing
        }
        let bridge = context.evaluateScript("TilinXSdkBridge.create({ send: __tilinxSend })")
        if let setupException {
