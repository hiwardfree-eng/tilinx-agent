...
enum JSRuntimeError: Error, Equatable, CustomStringConvertible {
    /// The bundled `tilinx-sdk.bridge.js` resource was not found in the app bundle.
    ...
    /// The evaluated bundle did not expose the expected `TilinXSdkBridge` global.
    case bridgeGlobalMissing
    /// `TilinXSdkBridge.create({ send })` did not return a usable bridge handle.
    ...
    var description: String {
        ...
        case .bridgeGlobalMissing:
            return "TilinXSdkBridge global not found after evaluating bundle"
        case let .bridgeCreateFailed(message):
            return "TilinXSdkBridge.create failed: \(message)"
