...
/// The TilinX helmet glyph as a SwiftUI `Shape`. The four `d` strings and the
/// small rect are copied VERBATIM from `ui/core/src/components/tilinx-avatar.tsx`
...
struct HelmetShape: Shape {
    ...
    /// The four helmet outline paths (verbatim from tilinx-avatar.tsx).
...
/// Agent avatar badge: a faint tinted circle with the TilinX helmet, optionally
/// wrapped in the running-glow halo. No initials, no photos — always the helmet
/// tinted by the agent's colour (PARITY §4). Mirrors `TilinXAvatar` in ui/core.
struct TilinXAvatar: View {
    @Environment(\.theme) private var theme
    /// The agent's themed hex `color` string; nil/unparseable → TilinX gray.
