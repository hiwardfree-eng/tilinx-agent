...
final class HexColorTests: XCTestCase {
    ...
    func testSixDigitHex() throws {
        let c = try XCTUnwrap(Color(tilinxHex: "#3b82f6"))
        let ch = channels(c)
    ...
    func testWithoutLeadingHash() {
        XCTAssertNotNil(Color(tilinxHex: "9b9b9b"))
    }

    func testThreeDigitShorthandExpands() throws {
        let short = try XCTUnwrap(Color(tilinxHex: "#f0a"))
        let long = try XCTUnwrap(Color(tilinxHex: "#ff00aa"))
    ...
    func testEightDigitAlpha() throws {
        let c = try XCTUnwrap(Color(tilinxHex: "#00000080"))
        XCTAssertEqual(channels(c).a, 128 / 255, accuracy: 0.02)
    }

    func testMalformedReturnsNil() {
        XCTAssertNil(Color(tilinxHex: "#12"))
        XCTAssertNil(Color(tilinxHex: "nothex"))
        XCTAssertNil(Color(tilinxHex: ""))
        XCTAssertNil(Color(tilinxHex: "#gggggg"))
    }
