...
//    user has something to send to support@gettilinx.ai instead of a blank.
//
...
(() => {
  ...
  var MESSAGES = {
    en: {
      title: "TilinX needs a newer version of macOS",
      body: "Please update your Mac to macOS Ventura (13) or later, then open TilinX again.",
    },
    es: {
      title: "TilinX necesita una versión más reciente de macOS",
      body: "Actualiza tu Mac a macOS Ventura (13) o una versión posterior y vuelve a abrir TilinX.",
    },
    pt: {
      title: "O TilinX precisa de uma versão mais recente do macOS",
      body: "Atualize seu Mac para o macOS Ventura (13) ou mais recente e abra o TilinX novamente.",
    },
  };
  ...
  var CRASH_MESSAGES = {
    en: {
      title: "TilinX couldn't start",
      body: "Try opening TilinX again. If this keeps happening, copy the details below and send them to hello@gettilinx.ai so we can fix it.",
      reload: "Reopen TilinX",
      copy: "Copy details",
      copied: "Copied",
    },
    es: {
      title: "TilinX no pudo iniciar",
      body: "Intenta abrir TilinX de nuevo. Si el problema continúa, copia los detalles y envíalos a hello@gettilinx.ai para que podamos resolverlo.",
      reload: "Volver a abrir TilinX",
      copy: "Copiar detalles",
      copied: "Copiado",
    },
    pt: {
      title: "O TilinX não conseguiu iniciar",
      body: "Tente abrir o TilinX novamente. Se o problema continuar, copie os detalhes e envie para hello@gettilinx.ai para que possamos resolver.",
      reload: "Reabrir o TilinX",
  ...
  function renderCrashScreen(root, message, diagnostics) {
    root.innerHTML =
      ...
      '<button id="tilinx-gate-reload" type="button" style="padding:8px 16px;font:inherit;font-size:14px;background:#3b82f6;color:#fff;border:0;border-radius:6px;cursor:pointer;">' +
      escapeHtml(message.reload) +
      "</button>" +
      '<button id="tilinx-gate-copy" type="button" style="padding:8px 16px;font:inherit;font-size:14px;background:#1f1f23;color:#e8e8ea;border:1px solid #2e2e33;border-radius:6px;cursor:pointer;">' +
      escapeHtml(message.copy) +
      "</button>" +
      "</div>" +
      '<pre id="tilinx-gate-diagnostics" style="margin:0;padding:12px;background:#1a1a1c;color:#a1a1aa;border:1px solid #2e2e33;border-radius:6px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:12px;line-height:1.5;white-space:pre-wrap;word-break:break-word;max-height:240px;overflow:auto;">' +
      escapeHtml(diagnostics) +
      "</pre>" +
      "</div></div>";

    var reloadBtn = root.querySelector("#tilinx-gate-reload");
    var copyBtn = root.querySelector("#tilinx-gate-copy");
    if (reloadBtn) {
    ...
    if (copyBtn) {
      copyBtn.addEventListener("click", () => {
        ...
        pre = root.querySelector("#tilinx-gate-diagnostics");
        if (
          pre &&
