...
//!
//! When the user double-clicks `TilinX.app` *inside the mounted DMG* (a
...
//! from `/Volumes/TilinX Installer/TilinX.app`. Workspaces, databases,
...
//!
//! Dev / debug builds also respond to `TILINX_FORCE_DMG_GUARD=1` so the
...
/// also honour `TILINX_FORCE_DMG_GUARD=1` so the dialog flow can be
/// tested with `pnpm tauri dev` without building + mounting a DMG.
pub fn should_run() -> bool {
    if cfg!(debug_assertions) && std::env::var("TILINX_FORCE_DMG_GUARD").as_deref() == Ok("1") {
        return true;
    }
...
fn show_move_dialog() -> Choice {
    let result = rfd::MessageDialog::new()
        ...
        .set_title("Move TilinX to your Applications folder")
        .set_description(
            "TilinX is currently running from the installer disk image. To use it normally, \
             move it to your Applications folder.\n\n\
             Click Move to do this automatically. Then open TilinX from Applications.",
        )
...
fn show_already_installed_and_running() {
    rfd::MessageDialog::new()
        ...
        .set_title("TilinX is already running")
        .set_description(
            "TilinX is already installed in your Applications folder and currently running. \
             Quit the running copy first, then drag TilinX from this disk image into Applications.",
        )
        .set_buttons(rfd::MessageButtons::Ok)
        .show();
}

/// True if there's a TilinX process running with `/Applications/TilinX.app` in its path.
/// Best-effort — we shell out to `pgrep` because there's no portable Rust API for this.
fn is_installed_copy_running() -> bool {
    Command::new("/usr/bin/pgrep")
        .args(["-f", "/Applications/TilinX.app/Contents/MacOS/"])
        .output()
        .map(|o| o.status.success() && !o.stdout.is_empty())
        .unwrap_or(false)
}

/// Copy the source `.app` bundle into `/Applications/TilinX.app`,
...
fn copy_to_applications(source_app: &Path) -> Result<PathBuf, String> {
    let dest = PathBuf::from("/Applications/TilinX.app");
    ...
    if dest.exists() {
        ...
        let rm = Command::new("/bin/rm")
            .args(["-rf", "/Applications/TilinX.app"])
            .status()
            .map_err(|e| format!("failed to invoke /bin/rm: {e}"))?;
        if !rm.success() {
            return Err(format!("rm -rf /Applications/TilinX.app exited with {rm}"));
        }
    }
...
pub fn handle_if_needed() {
    ...
    let forced_debug =
        cfg!(debug_assertions) && std::env::var("TILINX_FORCE_DMG_GUARD").as_deref() == Ok("1");
    ...
    match show_move_dialog() {
        ...
        Choice::Move => {
            if forced_debug {
                // Don't actually clobber /Applications/TilinX.app in dev.
            ...
            match copy_to_applications(&source_app) {
                Ok(dest) => {
                    if let Err(e) = launch_app(&dest) {
                        show_error(
                            "Couldn't launch TilinX from Applications",
                            &format!(
                                "TilinX was copied to /Applications but failed to launch. \
                ...
                Err(err) => {
                    show_error(
                        "Couldn't move TilinX to Applications",
                        &format!(
                            "TilinX couldn't copy itself to your Applications folder. \
                             Please drag the TilinX icon onto the Applications shortcut in the disk image instead.\n\n\
                             Details: {err}"
                        ),
                    );
                    std::process::exit(1);
...
mod tests {
    ...
    fn dmg_path_detected() {
        assert!(is_running_from_dmg(&PathBuf::from(
            "/Volumes/TilinX Installer/TilinX.app"
        )));
    }
    ...
    fn user_home_not_dmg() {
        assert!(!is_running_from_dmg(&PathBuf::from(
            "/Users/foo/Downloads/TilinX.app"
        )));
    }
    ...
    fn finds_enclosing_app() {
        let exe =
            PathBuf::from("/Volumes/TilinX Installer/TilinX.app/Contents/MacOS/tilinx-app");
        assert_eq!(
            enclosing_app_bundle(&exe),
            Some(PathBuf::from("/Volumes/TilinX Installer/TilinX.app"))
        );
    }

    #[test]
    fn no_enclosing_app_outside_bundle() {
        let exe = PathBuf::from("/Users/foo/target/debug/tilinx-app");
