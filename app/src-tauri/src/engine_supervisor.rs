...
//!
//! Spawns the Bun-compiled TilinX host as a child process, parses its stdout
//! for the `TILINX_HOST_LISTENING port=<p> token=<t>` banner, polls the host's
//! `/health` until ready, and hands the `{baseUrl, token}` handshake back to the
//! caller so the Tauri setup can inject `window.__TILINX_ENGINE__` before
//! showing the webview.
//!
//! Lifecycle:
...
//!   blocks forever and the engine orphaned (gettilinx/tilinx#306).
...
//!   backoff and emits a `tilinx-event` toast to the webview on each
//!   restart.
//!
...
/// client). Tagged so the shared `tilinx-app` project can isolate engine
    ...
    sentry::with_scope(
        ...
        || {
            sentry::capture_message(
                &format!(
                    "tilinx-engine subprocess exited unexpectedly (exit {exit:?}); \
...
impl EngineSubprocess {
    /// Spawn `tilinx-engine` and wait up to `timeout` for the banner.
    ///
    /// `env` is merged on top of the inherited environment — used by the
    /// TilinX app to pass product-layer prompts (`TILINX_APP_SYSTEM_PROMPT`,
    /// `TILINX_APP_ONBOARDING_PROMPT`) into the engine at boot so the engine
    ...
    pub fn spawn(
        ...
        cmd.stdout(Stdio::piped())
            ...
            // force-quits.
            .stdin(Stdio::piped());
        // The supervised marker, set right next to the pipe it describes so the
        ...
        #[cfg(windows)]
        {
            ...
            // `tilinx-app.exe` is a GUI Tauri process with no
            ...
            // every time TilinX launches. Tauri's own `Sidecar`
        ...
        // engine orphaned (gettilinx/tilinx#306). Assigned immediately
        ...
        thread::spawn(move || {
            let logs_dir = crate::tilinx_dir().join("logs");
...
/// `tilinx-engine`, kept from the Rust-engine era).
///
/// Resolution order:
/// 1. `TILINX_ENGINE_BIN` env var (dev override / SSH deploy).
/// 2. Sibling of the current executable — this is where Tauri's
///    `externalBin` places sidecars in shipped app bundles:
///      - macOS: `TilinX.app/Contents/MacOS/tilinx-engine`
///      - Windows: next to `tilinx-app.exe`
...
/// 3. `<resource_dir>/binaries/tilinx-engine` — legacy / belt-and-braces
///    fallback for platforms that stage externalBin into the resources
///    tree.
///
/// The cargo target dir (`target/{debug,release}/tilinx-engine`) is
...
pub fn resolve_engine_binary(resource_dir: Option<&PathBuf>) -> Result<PathBuf, String> {
    ...
    if let Ok(p) = std::env::var("TILINX_ENGINE_BIN") {
...
/// Spawn a background thread that keeps `tilinx-engine` alive. On crash,
...
fn parse_banner(line: &str) -> Option<EngineHandshake> {
    // The TilinX host emits `TILINX_HOST_LISTENING port=<p> token=<t>`.
    // (The Rust engine's `TILINX_ENGINE_LISTENING` prefix is gone with the
    // engine — accepting it could only ever bless a stale wrong binary.)
    let rest = line.strip_prefix("TILINX_HOST_LISTENING ")?;
...
/// the engine orphaned (gettilinx/tilinx#306). A kill-on-close job is
...
/// be handed a concrete `TILINX_HOST_PORT` (it echoes the *requested* port in
...
/// Poll the TilinX host's `/health` until a 2xx response, or timeout. Served as
...
pub fn wait_until_host_healthy(
    ...
    wait_until_healthy_at(handshake, "/health", timeout)
}

/// Poll `<base_url><path>` until a 2xx response, or timeout. Bearer-authed.
fn wait_until_healthy_at(
    handshake: &EngineHandshake,
    path: &str,
    timeout: Duration,
) -> Result<(), String> {
    let client = reqwest::blocking::Client::new();
    let url = format!("{}{}", handshake.base_url(), path);
    let deadline = Instant::now() + timeout;
...
mod tests {
    ...
    fn parses_host_banner() {
        // The TilinX host emits the TILINX_HOST_LISTENING banner.
        let h = parse_banner("TILINX_HOST_LISTENING port=4318 token=deadbeef").unwrap();
    ...
    fn rejects_legacy_engine_banner() {
        ...
        assert!(parse_banner("TILINX_ENGINE_LISTENING port=12345 token=abc").is_none());
    }
