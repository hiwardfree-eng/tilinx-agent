...
//! brings TilinX to the foreground and replays the pending mission nav.
//!
...
//! to a finished mission whenever they refocus TilinX.

use tauri::AppHandle;

/// Show a native notification whose click raises TilinX and emits
...
mod windows {
    ...
    /// `com.tilinx.app` AUMID so the toast carries TilinX's icon + name;
    ...
    mod tests {
        ...
        fn release_uses_app_identifier_and_dev_uses_powershell() {
            // Release: the toast must carry TilinX's registered AUMID so the
            // OS shows our icon/name and routes the click back to us.
            assert_eq!(
                resolve_toast_app_id(false, "com.tilinx.app").as_str(),
                "com.tilinx.app",
            );
            // Dev: no AUMID is registered, so fall back to PowerShell's.
            assert_eq!(
                resolve_toast_app_id(true, "com.tilinx.app").as_str(),
