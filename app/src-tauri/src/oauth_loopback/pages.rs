//! The two self-contained pages the loopback serves back into the browser.
//!
//! The listener serves no other assets, so the TilinX helmet is inlined as SVG
//! (no `<img src>` to 404) and the "Open TilinX" button is a `tilinx://open`
//! deep link that focuses the app (handled in `lib.rs`). Copy matches the
//! English-only sign-in flow; when that flow gets i18n these strings move with it.
//!
//! Two outcomes, two pages: the redirect we were waiting for gets
//! {@link success_page}; a redirect carrying somebody ELSE's `state` (a restored
//! tab replaying an old consent) gets {@link stale_page} and the listener stays
//! up for the real one.

/// Uses the `r##"…"##` delimiter because the inlined SVG contains
/// `fill="#161615"` — the `"#` sequence would close a plain `r#"…"#`.
const LOGO_SVG: &str = r##"<svg class="logo" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 412.248 448.898" role="img" aria-label="TilinX">
      <defs>
...
/// One page shell shared by both outcomes, so they cannot drift apart visually.
fn page(title: &str, heading: &str, body: &str, button: bool) -> String {
    let action = if button {
        r#"<a class="btn" href="tilinx://open">Open TilinX</a>"#
    } else {
        ""
    };
    format!(
...
/// Served for the redirect this attempt was waiting for.
pub fn success_page() -> String {
    page(
        "TilinX \u{2014} Signed in",
        "You're signed in",
        "You can close this tab and return to TilinX.",
        true,
    )
}

/// Served for a redirect whose `state` belongs to an older attempt. The listener
/// KEEPS waiting after this, so the user's real sign-in is not consumed.
pub fn stale_page() -> String {
    page(
        "TilinX \u{2014} Old sign-in tab",
        "This sign-in link has expired",
        "It came from an earlier sign-in attempt. You can close this tab and finish signing in from the TilinX window.",
        false,
    )
}
...
mod tests {
    ...
    fn success_page_is_self_contained() {
        ...
        // gets fetched from the loopback (which serves only these pages).
    ...
    #[test]
    fn success_page_has_open_tilinx_deep_link() {
        // The "Open TilinX" button focuses the app via the `tilinx://`
        // scheme (handled in lib.rs), not an http URL.
        assert!(success_page().contains(r#"href="tilinx://open""#));
    }

    #[test]
    fn stale_page_explains_itself_and_offers_no_action() {
        // The stale tab must not invite the user back into the app as if it had
        // signed them in — the real attempt is still waiting in the TilinX window.
        let page = stale_page();
        assert!(page.contains("expired"));
        assert!(!page.contains("tilinx://open"));
        assert!(page.contains("<svg"));
    }
}
