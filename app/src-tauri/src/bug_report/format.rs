...
pub(super) fn format_issue_title(payload: &BugReportPayload) -> String {
    ...
    if let Some(message) = payload
        ...
        .filter(|s| !s.is_empty())
    {
        return truncate_chars(&format!("TilinX feedback: {message}"), 140);
    }

    let command = collapse_whitespace(&payload.command);
    let summary = collapse_whitespace(payload.error.lines().next().unwrap_or(""));
    if summary.is_empty() {
        truncate_chars(&format!("TilinX bug: {command}"), 140)
    } else {
        truncate_chars(&format!("TilinX bug: {command} - {summary}"), 140)
    }
}
...
mod tests {
    ...
    fn issue_title_includes_command_and_error_summary() {
        ...
        assert_eq!(
            ...
            "TilinX bug: list_workspaces - Error: no workspace found"
        );
    }
    ...
    fn user_message_leads_title_when_present() {
        ...
        assert_eq!(
            title,
            "TilinX feedback: The chat froze when I asked about taxes"
        );
    }
    ...
    fn empty_user_message_falls_through_to_command_title() {
        ...
        assert!(title.starts_with("TilinX bug:"));
    }
}
