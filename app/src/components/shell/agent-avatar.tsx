...
/** Pick the right TilinX logo SVG for a background color */
export function getTilinXLogo(bgColor?: string): string {
  if (!bgColor) return tilinxIcon;
  return isLightColor(bgColor) ? tilinxIcon : tilinxIconWhite;
}
...
/** TilinX helmet that inherits text color via currentColor.
 *  Use this everywhere instead of <img src={tilinxIcon}>. */
export function TilinXLogo({
  size = 20,
  className = "",
}: {
  size?: number;
  ...
  return (
    <svg
      ...
      fill="currentColor"
      role="img"
      aria-label="TilinX logo"
    >
      <title>TilinX logo</title>
