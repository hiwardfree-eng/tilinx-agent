...
Take `redirect_url` from response, present to user as markdown link with `#tilinx_toolkit=googlesheets` appended (so TilinX render connect card). Wait for approval before continuing.
